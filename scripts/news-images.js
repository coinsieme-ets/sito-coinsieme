...
async function inspectSource(source,{fetcher=fetch}={}){
 ...
 for(const candidate of imageCandidates(html,response.url||url)){
  try{const r=await fetcher(candidate.url,{signal:AbortSignal.timeout(20000)});if(!r.ok||!/^image\//i.test(r.headers.get('content-type')||''))throw Error('Immagine HTTP '+r.status+' o MIME non valido');const buffer=await bytes(r,10000000),meta=await require("sharp")(buffer).metadata();if(!['jpeg','png','webp','avif','gif'].includes(meta.format)||meta.width<200||meta.height<100)throw Error('Formato o dimensioni non idonei');return {...report,original:candidate.url,kind:candidate.kind,imageStatus:r.status,width:meta.width,height:meta.height,hash:crypto.createHash('sha256').update(buffer).digest('hex'),format:meta.format,buffer};
  }catch(e){report.attempts.push({image:candidate.url,error:e.message});}
 }
 report.reason='Nessuna immagine OpenGraph/Twitter valida';
 }catch(e){report.reason=e.message;}return report;
}
