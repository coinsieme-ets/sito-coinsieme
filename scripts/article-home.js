...
function selectHomeArticles(items,today=todayRome()){
 ...
 .sort((a,b)=>{
  const level=tier(a)-tier(b);
  if(level)return level;
  // Only active editorial prominence overrides publication chronology.
  // Expired prominence returns to the same date ordering as normal articles.
  const editorial=tier(a)<levels.normale ? pinned(a)-pinned(b)||a.ordine_home-b.ordine_home : 0;
  return editorial||(b.date||'').localeCompare(a.date||'')||a.slug.localeCompare(b.slug);
 });
}
